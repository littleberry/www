...
if (isset($_POST["action"]) and $_POST["action"] == "edit_client") {
	...
	processEditClient($client_id);
} else {
	displayClientUpdateForm(array(), array(), new Client(array()));
}


function displayClientUpdateForm($errorMessages, $missingFields, $client) {
	//the user came in from the manage page, get the clients information only then. Otherwise, get the data from the page post values.
	$client_id = isset($_GET["client_id"]) ? $_GET["client_id"] : 0;
	if ($client_id != 0) {
		//this is kindof hackey, fix this.
		$client = Client::getClient($client_id);
	} else {
		$client_id = isset($_POST["client_id"]) ? $_POST["client_id"] : 0;
	}
	displayPageHeader("Edit Client");
	if ($errorMessages) {
	...
	?>
	 <form action = "edit_client.php" method="post" style="margin-bottom:50px;">
    	...
    	<?php } else {?>
        	<label for="client_address"<?php validateField("client_address", $missingFields)?>>Client Address</label>
   			<textarea cols="5" rows="10" name="client_address" id="client_address" value="<?php echo $client->getValueEncoded("client_address")?>" /><?php echo $client->getValueEncoded("client_address")?></textarea>
    	<?php } ?>
...
function processEditClient($client_id) {
	if (!$client_id) {
		echo "horrible failure, we don't have a client id.";
		exit;
	}
	...
	if ($errorMessages) {
		displayClientUpdateForm($errorMessages, $missingFields, $client);
	} else {
...
function displayClients() {
	//displayPageHeader("You are now a client, but this should be shown in the view clients page");
